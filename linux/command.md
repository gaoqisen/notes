...
passwd  // 重置密码
reboot  // 重启（root用户才有权限）
sudo   // 临时获取root权限
tar -czvf file.tar.gz .  // 压缩当前文件夹
...
yum install gcc gcc-c++  安装gcc和g++（centos下）
```
...
tar -xvf archive.tar.gz 解压一个gzip格式的压缩包 
...
```
关闭系统使用Shutdown命令，确保用户和系统的资料完整。只有root用户才能使用这个命令。
一般的用户是不允许执行这个命令的。
我们先看看showdown语法：
