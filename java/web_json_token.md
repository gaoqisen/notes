...
token是服务器端生成的一串字符串，作为服务端访问服务端的一个令牌。第一次登录的的是就为客服端生成一个token，后期用户登录的时候就传递这个token用于验证，无需带上用户的用户的帐号和密码。
...
- 多平台跨域: CORS(跨域资源共享)，对应用程序和服务进行扩展的时候，需要介入各种各种的设备和应用程序。只要用户有一个通过了验证的token，数据和资源就能够在任何域上被请求到。
...
- 由最大的缺点是由于服务器不保存 session 状态，因此无法在使用过程中废止某个 token，或者更改 token 的权限。也就是说，一旦 JWT 签发了，在到期之前就会始终有效，除非服务器部署额外的逻辑
