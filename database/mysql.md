...
| 主主复制管理 MMM（Master-Master replication manager for MySQL） | 2007～2008 | 5.0+ | Open Query公司开发的简化主主复制故障切换、提高可用性                   |
| 集群高可用 MHA（MySQL Master High Availability）             | 2010～2011 | 5.1+    | Yahoo公司开发的减少切换主从速度、自动修复复制关系                 |
| 半同步                                                       |   2010       |     5.5+    |   官方数据一致性要求高                                 |
...
| 双主复制架构DMR |   双主互相备份，实现高可用    |   写入冲突需应用程序解决，切换复杂    | 两个主节点互相写入数据，互相复制对方的变更     |
| 主主复制管理MMM |  主主复制，避免单点故障  | 部署维护复制     |  通过监视复制延迟实现自动故障切换    |
