...
<p>2.1 Buffer Pool（缓冲池）</p>
<p>缓冲池是 innodb 的一个内存区域，可以理解为是磁盘数据里面的数据中转站。举个例子比如磁盘是 50G, 内存 8G，在查询数据时如果没有索引就会进行全表扫描，那么在扫描的时候不能把磁盘里面的数据全部加载到内存中，缓存池的好处</p>
...
<h3 id="_2-2-double-write-buffer-双写缓冲区" tabindex="-1"><a class="header-anchor" href="#_2-2-double-write-buffer-双写缓冲区" aria-hidden="true">#</a> 2.2 Double write Buffer(双写缓冲区)</h3>
<p>双写缓冲区的作用主要是解决在 buffer pool 往磁盘写数据的时候，由于是随机写一页的数据可能有部分写成功了，部分没有写成功。这个时候有了 Double write Buffer 就可以在重启服务的时候通过 Double write Buffer 恢复数据。具体做法如下：</p>
...
<h3 id="_2-3-adaptive-hash-index-自适应哈希" tabindex="-1"><a class="header-anchor" href="#_2-3-adaptive-hash-index-自适应哈希" aria-hidden="true">#</a> 2.3 Adaptive Hash Index(自适应哈希)</h3>
<p>在 innodb 中是没有哈希数据结构的，但是如果出现很多高频相同的查询，这个时候 mysql 就会根据查询次数、系统负载情况、实际的查询模式等进行自动优化。将相同的查询自适应为 哈希，这样就能提高查询效率，降低系统开销。如果在系统负载高，sql 语句相同查询低则会删除自适应哈希。目前没有可修改的参数，是 mysql 自己进行的自适应优化。</p>
